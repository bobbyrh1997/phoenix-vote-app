defmodule VoteApp.Auth.LoginToken do
  use Ecto.Schema
  import Ecto.Query
  alias VoteApp.Auth.LoginToken
  ...
  schema "logins_tokens" do
    field :token, :binary
    field :context, :string
    field :sent_to, :string
    belongs_to :login, VoteApp.Auth.Login
  ...
  def build_session_token(login) do
    ...
    {token, %LoginToken{token: token, context: "session", login_id: login.id}}
  end
      ...
      {:ok, decoded_token} ->
        ...
        query =
          from token in token_and_context_query(hashed_token, context),
            join: login in assoc(token, :login),
            where: token.inserted_at > ago(^days, "day") and token.sent_to == login.email,
            select: login
  ...
  def token_and_context_query(token, context) do
    from LoginToken, where: [token: ^token, context: ^context]
  end
  ...
  def login_and_contexts_query(login, :all) do
    from t in LoginToken, where: t.login_id == ^login.id
  end

  def login_and_contexts_query(login, [_ | _] = contexts) do
    from t in LoginToken, where: t.login_id == ^login.id and t.context in ^contexts
  end
end
