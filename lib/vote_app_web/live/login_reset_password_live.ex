defmodule VoteAppWeb.LoginResetPasswordLive do
  ...
  def handle_event("reset_password", %{"login" => login_params}, socket) do
    case Auth.reset_login_password(socket.assigns.login, login_params) do
      {:ok, _} ->
        {:noreply,
         socket
  ...
  def handle_event("validate", %{"login" => login_params}, socket) do
    changeset = Auth.change_login_password(socket.assigns.login, login_params)
    {:noreply, assign_form(socket, Map.put(changeset, :action, :validate))}
  end

  defp assign_login_and_token(socket, %{"token" => token}) do
    if login = Auth.get_login_by_reset_password_token(token) do
