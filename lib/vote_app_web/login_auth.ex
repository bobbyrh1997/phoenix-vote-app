defmodule VoteAppWeb.LoginAuth do
  use VoteAppWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_vote_app_web_login_remember_me"
  ...
  def log_out_login(conn) do
    ...
    login_token && Auth.delete_login_session_token(login_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      VoteAppWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_login(conn, _opts) do
    ...
    login = login_token && Auth.get_login_by_session_token(login_token)
  ...
  the current_login:
      ...
      defmodule VoteAppWeb.PageLive do
        use VoteAppWeb, :live_view

        on_mount {VoteAppWeb.LoginAuth, :mount_current_login}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{VoteAppWeb.LoginAuth, :ensure_authenticated}] do
  ...
  defp mount_current_login(session, socket) do
    Phoenix.Component.assign_new(socket, :current_login, fn ->
      if login_token = session["login_token"] do
        Auth.get_login_by_session_token(login_token)
      end
    end)
  end
