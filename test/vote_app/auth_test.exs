defmodule VoteApp.AuthTest do
  use VoteApp.DataCase

  alias VoteApp.Auth

  import VoteApp.AuthFixtures
  alias VoteApp.Auth.{Login, LoginToken}

  describe "get_login_by_email/1" do
    test "does not return the login if the email does not exist" do
      refute Auth.get_login_by_email("unknown@example.com")
    end

    test "returns the login if the email exists" do
      %{id: id} = login = login_fixture()
      assert %Login{id: ^id} = Auth.get_login_by_email(login.email)
    end
  end

  describe "get_login_by_email_and_password/2" do
    test "does not return the login if the email does not exist" do
      refute Auth.get_login_by_email_and_password("unknown@example.com", "hello world!")
    end

    test "does not return the login if the password is not valid" do
      login = login_fixture()
      refute Auth.get_login_by_email_and_password(login.email, "invalid")
    end
    ...
    test "returns the login if the email and password are valid" do
      ...
      assert %Login{id: ^id} =
               Auth.get_login_by_email_and_password(login.email, valid_login_password())
    end
  end
  ...
  describe "get_login!/1" do
    ...
    test "returns the login with the given id" do
      %{id: id} = login = login_fixture()
      assert %Login{id: ^id} = Auth.get_login!(login.id)
    end
  end

  describe "register_login/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Auth.register_login(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Auth.register_login(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Auth.register_login(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Auth.register_login(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Auth.register_login(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers logins with a hashed password" do
      email = unique_login_email()
      {:ok, login} = Auth.register_login(valid_login_attributes(email: email))
  ...
  describe "change_login_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Auth.change_login_registration(%Login{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Auth.change_login_registration(
          %Login{},
  ...
  describe "change_login_email/2" do
    test "returns a login changeset" do
      assert %Ecto.Changeset{} = changeset = Auth.change_login_email(%Login{})
  ...
  describe "apply_login_email/3" do
    ...
    test "requires email to change", %{login: login} do
      {:error, changeset} = Auth.apply_login_email(login, valid_login_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{login: login} do
      {:error, changeset} =
        Auth.apply_login_email(login, valid_login_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{login: login} do
      ...
      {:error, changeset} =
        Auth.apply_login_email(login, valid_login_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{login: login} do
      ...
      {:error, changeset} = Auth.apply_login_email(login, password, %{email: email})
    ...
    test "validates current password", %{login: login} do
      {:error, changeset} =
        Auth.apply_login_email(login, "invalid", %{email: unique_login_email()})
    ...
    test "applies the email without persisting it", %{login: login} do
      email = unique_login_email()
      {:ok, login} = Auth.apply_login_email(login, valid_login_password(), %{email: email})
      assert login.email == email
      assert Auth.get_login!(login.id).email != email
    end
  end
    ...
    test "sends token through notification", %{login: login} do
      token =
        extract_login_token(fn url ->
          Auth.deliver_login_update_email_instructions(login, "current@example.com", url)
        end)
  ...
  describe "update_login_email/2" do
    setup do
      ...
      token =
        extract_login_token(fn url ->
          Auth.deliver_login_update_email_instructions(%{login | email: email}, login.email, url)
        end)
    ...
    test "updates the email with a valid token", %{login: login, token: token, email: email} do
      assert Auth.update_login_email(login, token) == :ok
    ...
    test "does not update email with invalid token", %{login: login} do
      assert Auth.update_login_email(login, "oops") == :error
    ...
    test "does not update email if login email changed", %{login: login, token: token} do
      assert Auth.update_login_email(%{login | email: "current@example.com"}, token) == :error
      assert Repo.get!(Login, login.id).email == login.email
      assert Repo.get_by(LoginToken, login_id: login.id)
    end
    ...
    test "does not update email if token expired", %{login: login, token: token} do
      ...
      assert Auth.update_login_email(login, token) == :error
      assert Repo.get!(Login, login.id).email == login.email
      assert Repo.get_by(LoginToken, login_id: login.id)
    end
  end
  ...
  describe "change_login_password/2" do
    test "returns a login changeset" do
      assert %Ecto.Changeset{} = changeset = Auth.change_login_password(%Login{})
      assert changeset.required == [:password]
    end

    test "allows fields to be set" do
      changeset =
        Auth.change_login_password(%Login{}, %{
  ...
  describe "update_login_password/3" do
    ...
    test "validates password", %{login: login} do
      {:error, changeset} =
        Auth.update_login_password(login, valid_login_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{login: login} do
      ...
      {:error, changeset} =
        Auth.update_login_password(login, valid_login_password(), %{password: too_long})
    ...
    test "validates current password", %{login: login} do
      {:error, changeset} =
        Auth.update_login_password(login, "invalid", %{password: valid_login_password()})
    ...
    test "updates the password", %{login: login} do
      ...
      assert is_nil(login.password)
      assert Auth.get_login_by_email_and_password(login.email, "new valid password")
    end

    test "deletes all tokens for the given login", %{login: login} do
      _ = Auth.generate_login_session_token(login)
  ...
  describe "generate_login_session_token/1" do
    ...
    test "generates a token", %{login: login} do
      token = Auth.generate_login_session_token(login)
  ...
  describe "get_login_by_session_token/1" do
    ...
    test "returns login by token", %{login: login, token: token} do
      assert session_login = Auth.get_login_by_session_token(token)
      assert session_login.id == login.id
    end

    test "does not return login for invalid token" do
      refute Auth.get_login_by_session_token("oops")
    end

    test "does not return login for expired token", %{token: token} do
      {1, nil} = Repo.update_all(LoginToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Auth.get_login_by_session_token(token)
    end
  end
  ...
  describe "delete_login_session_token/1" do
    test "deletes the token" do
      login = login_fixture()
      token = Auth.generate_login_session_token(login)
      assert Auth.delete_login_session_token(token) == :ok
      refute Auth.get_login_by_session_token(token)
    end
  end
  ...
  describe "confirm_login/1" do
    ...
    test "confirms the email with a valid token", %{login: login, token: token} do
      assert {:ok, confirmed_login} = Auth.confirm_login(token)
    ...
    test "does not confirm with invalid token", %{login: login} do
      assert Auth.confirm_login("oops") == :error
    ...
    test "does not confirm email if token expired", %{login: login, token: token} do
      {1, nil} = Repo.update_all(LoginToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Auth.confirm_login(token) == :error
      refute Repo.get!(Login, login.id).confirmed_at
      assert Repo.get_by(LoginToken, login_id: login.id)
    end
  end
  ...
  describe "deliver_login_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{login: login} do
      token =
        extract_login_token(fn url ->
          Auth.deliver_login_reset_password_instructions(login, url)
        end)
  ...
  describe "reset_login_password/2" do
    ...
    test "validates password", %{login: login} do
      {:error, changeset} =
        Auth.reset_login_password(login, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{login: login} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Auth.reset_login_password(login, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{login: login} do
      {:ok, updated_login} = Auth.reset_login_password(login, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given login", %{login: login} do
      _ = Auth.generate_login_session_token(login)
      {:ok, _} = Auth.reset_login_password(login, %{password: "new valid password"})
      refute Repo.get_by(LoginToken, login_id: login.id)
    end
  end
