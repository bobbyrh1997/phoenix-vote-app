defmodule VoteAppWeb.LoginAuthTest do
  use VoteAppWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias VoteApp.Auth
  alias VoteAppWeb.LoginAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, VoteAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_login/3" do
    test "stores the login token in the session", %{conn: conn, login: login} do
      ...
      assert Auth.get_login_by_session_token(token)
    end
  ...
  describe "logout_login/1" do
    test "erases session and cookies", %{conn: conn, login: login} do
      login_token = Auth.generate_login_session_token(login)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Auth.get_login_by_session_token(login_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "logins_sessions:abcdef-token"
      VoteAppWeb.Endpoint.subscribe(live_socket_id)
